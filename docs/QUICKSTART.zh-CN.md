...
PARAMETERS > PSET 可以保存和调用全部控制。上次用过的那一组会在下次加载
grainloom 时自动读回，并且**早于任何值被推送到引擎**，所以引擎从保存的值开始。
一次都没保存过时不会有任何变化。PSET 存的是控制值，不是 Buffer 里的声音。

在 SELECT 界面里可以看到这个乐器的简介和按键说明，E2 翻动。
